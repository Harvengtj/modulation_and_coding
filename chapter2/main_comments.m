...
ppm = 1*(1e-6);         % PPM
phi_0 = 0.1;            % Phase offset %unifrnd(0, 2*pi); 
K = 0.05;               % Kappa
...
t_axis = zeros(size(signal_rx, 1), size(signal_rx, 2));
for i = 1:length(EbN0)
    t_axis(i,:) = t;
end
...
% Compensation
t_compensate =  t(1, N/2:end-(N/2)); % Time axis for compensation
t_axis_compensate = zeros(size(cropped_filtered_signal_rx, 1), size(cropped_filtered_signal_rx, 2));
for i = 1:length(EbN0)
    t_axis_compensate(i,:) = t_compensate;
end
