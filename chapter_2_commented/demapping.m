...
switch modulation,
    ...
    case 'qam'
        ...
        % Integer detection
        int_detI = round(int_rxI); % Round the float because of noise
        int_detI(find(int_detI<0)) = 0; % Case where int < 0 (Find(#condition) returns an index)
        int_detI(find(int_detI>2^NbpsI-1)) = 2^NbpsI-1; % Case where int > 3
