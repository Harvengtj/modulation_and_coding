...
EbN0 = 0:2:20;          % Vector of Eb/N0 ratios (energy per bit to noise PSD) for BER comparison
...
Fc = 600e6;             % Carrier frequency
ppm = 1*(1e-6);         % PPM
phi_0 = 2*pi*rand; % Phase offset 
K = 0.05;               % Kappa
timeShift = [2 4 6 8 10];          % Time shift
nb_iter = 5;

iter_signal_rx = zeros(nb_iter*length(EbN0),Nsymb);
iter_bits_rx = zeros(nb_iter*length(EbN0),Nb);
iter_bits_tx = zeros(nb_iter,Nb);
...
% Add bits_tx to iter_bits_tx
iter_bits_tx(iter,:) = bits_tx';
...
% % CFO computation
% t = (0:size(signal_rx, 2) - 1)*(1/Fs); % Time axis
% t_axis = repelem(t,length(EbN0),1);
% 
% CFO = exp(1i*2*pi*(ppm(iter)*Fc)*t_axis); % CFO
% 
% % CPE computation
% CPE = exp(1i*phi_0); % CPE
% 
% % Total shift
% shift = CFO.*CPE;
% 
% % Effect on the received signal
% signal_rx = signal_rx .* shift;
...
for i = 1:length(EbN0)
    ...
    %cropped_filtered_signal_rx(i,:) = filtered_signal_rx(i, N:end-(N-1)); % Crop the filtered signal to remove extensions (causality)
end

% %% === Compensation (CFO + CPE) ===
% fprintf('\n=== Compensation (CFO + CPE) ===\n');
% 
% % Compensation
% t_compensate =  t(1, N/2:end-(N/2)); % Time axis for compensation (we need to remove time instances because the size of our signal has N samples less than before)
% t_axis_compensate = repelem(t_compensate, length(EbN0),1);
% 
% % Compensation
% cropped_filtered_signal_rx = cropped_filtered_signal_rx .* exp(-1i*2*pi*(ppm*Fc)*t_axis_compensate -1i*phi_0);
% 
%% === Time Shifting ===
...
%t2 = ((0:Nsymb*M-1))*1/Fs;
for i = 1:length(EbN0)
    shifted_signal_rx(i,:)=circshift(filtered_signal_rx(i,:),timeShift(iter));
    cropped_filtered_signal_rx(i,:) = shifted_signal_rx(i,N:end-(N-1));
    %cropped_filtered_signal_rx(i,:) = cropped_filtered_signal_rx(i,:).*exp(-1j*(2*pi*CFO.*t2));
end


%% === Gardner ===
downsampled_corrected_signal_rx = zeros(length(EbN0),Nsymb);
for i = 1:length(EbN0)
    [downsampled_corrected_signal_rx(i,:),time_error] = gardner(cropped_filtered_signal_rx(i,:),K,M); % Oversample by 2 
end
...
% Add to iter_signal_rx
iter_signal_rx((iter-1)*length(EbN0)+1:iter*length(EbN0), 1:Nsymb) = downsampled_corrected_signal_rx;

%% === Demapping ===
downsampled_corrected_signal_rx = downsampled_corrected_signal_rx';                         % Transpose the downsampled signal matrix
bits_rx = zeros(Nb, length(EbN0));                                      % Initialize a matrix for the received bits

for i = 1:length(EbN0)
    bits_rx(:, i) = demapping_comments(downsampled_corrected_signal_rx(:, i), Nbps, modulation); % Demap symbols to bits
end

% Add to iter_bit_rx
iter_bits_rx((iter-1)*length(EbN0)+1:iter*length(EbN0), 1:Nb) = bits_rx';
...
berEst = zeros(nb_iter, length(EbN0));                                           % Initialize a vector for the estimated BER

for iter = 1:nb_iter

    for i = 1:length(EbN0)
        nErrors = biterr(iter_bits_tx(iter,:),iter_bits_rx((iter-1)*length(EbN0)+i,:));  % Calculate the number of bit errors
        berEst(iter,i) = nErrors/Nb;                                        % Estimate the BER
    end
...
legend('t_0 = 0.02Tsymb', 't_0 = 0.04Tsymb', 't_0 = 0.06Tsymb', 't_0 = 0.08Tsymb', 't_0 = 0.1Tsymb', 'Location','southwest');                             % Add a legend
xlabel('Eb/N0 (dB)');                                                   % Label the x-axis
ylabel('Bit Error Rate');                                               % Label the y-axis
title('BER vs Eb/N0');                                                  % Add a title to the plot

% Plot timeShift = 0.1*Tsymb for 20 dB
scatterplot(iter_signal_rx(55,:))
title(['Constellation at Eb/N0 = 20 dB']);